...
Before being saved to the index or used for querying, every embedding vector (whether it's from a color histogram, ResNet50, or Swin Transformer) undergoes **L2 normalization**.
...
## 3. Hybrid Score Fusion (Linear Interpolation)
In the `hybrid_search_similar` function, the system combines the search results from two different models (CNN ResNet50 and Swin Transformer) to get a more accurate result. This is done using a weighted mathematical combination called **Linear Interpolation**.

**Formula:**
Given a weight parameter $\alpha$ (which defaults to $0.5$), the final hybrid score for an image is calculated as:
$$ S_{hybrid} = \alpha \times S_{cnn} + (1 - \alpha) \times S_{swin} $$
Where $S_{cnn}$ and $S_{swin}$ are the cosine similarity scores from the respective models. This allows you to smoothly bias the search results more towards the CNN or the Transformer depending on the value of $\alpha$.
*(Found in `src/search.py`)*
